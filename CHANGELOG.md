...
- All SQLModel crates use Asupersync 0.5.0, including the re-exported `Cx` and
  `Outcome` types. Internal SQLModel dependencies advance together to 0.5.0.
...
- **`caching_sha2_password` logins (the MySQL 8 default) always failed** with `Protocol error:
  Unknown additional auth response: 01`. The server frames the fast-auth status as an AuthMoreData
...
- **SCRAM-SHA-256 logins (the Postgres 14+ default) always failed** with `Protocol error:
  Unexpected message during auth: ParameterStatus { name: "in_hot_standby", .. }`. The SCRAM
...
- `sqlmodel_schema::Dialect` is now the core `sqlmodel_core::Dialect` (re-exported) instead of a
  second, identical-looking enum. `Introspector::new(conn.dialect())` and `SchemaBuilder::new()
  .dialect(conn.dialect())` now type-check; the only observable difference is that
...
Final tree of the 2026-09-02 reality-check session (builds offloaded with `rch exec`, database suites
run locally against Docker because remote workers cannot reach the containers):

- `cargo fmt --all --check`: clean.
- `cargo clippy --workspace --all-targets -- -D warnings`: clean.
- `RUSTDOCFLAGS=-D warnings cargo doc --workspace --no-deps`: clean (two intra-doc links found by
  the first pass were fixed and the docs rebuilt without warnings).
- `cargo test --workspace --no-fail-fast`: 61 suites, 2041 passed, 0 failed, 329 ignored (all
  pre-existing `#[ignore]`s; none added). `cargo test --workspace --doc`: 52 passed.
- `cargo audit --deny warnings`: clean with the ignores in `.cargo/audit.toml`.
  `cargo deny check advisories bans licenses sources`: all ok.
- Against live databases (postgres:16 with scram-sha-256, mysql:8.4 with caching_sha2_password):
  `sqlmodel-postgres` integration 7/7, `sqlmodel-mysql` integration 7/7, `sqlmodel-e2e` smoke,
  migrations, and concurrent-writers each passing on C SQLite (memory, file), FrankenSQLite,
  PostgreSQL, and MySQL. MariaDB and TLS were not exercised locally. GitHub Actions runs for the
