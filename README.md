...
async fn adult_heroes(cx: &Cx, conn: &impl Connection) -> Outcome<Vec<Hero>, Error> {
    ...
    // Render the SQL for any dialect without executing it
    let (sql, _params) = query.build_with_dialect(Dialect::Postgres);
    assert_eq!(
        sql,
...
(`BYTEA` for byte fields on PostgreSQL, `BINARY(16)` for UUIDs on MySQL, and so on).
...
| `uuid::Uuid` | `UUID` | `uuid` |
| `rust_decimal::Decimal` | `NUMERIC` / `DECIMAL` (scale preserved) | `decimal` |
