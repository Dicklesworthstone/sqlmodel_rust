...
If rch or its workers are unavailable, it fails open — builds run locally as normal. Exception observed 2026-09-01: when the global RCH config has `force_remote = true` and no worker is admissible (pressure-blocked fleet), the PreToolUse hook refuses plain `cargo test|check|clippy|doc` with "remote required; refusing local fallback". Invoking the same command explicitly as `rch exec -- cargo <cmd>` is admitted and runs (remotely when a worker is available, locally otherwise). Do not flip `force_remote` to work around it. Note that the rch shim is also installed over the toolchain's own `bin/cargo`, so absolute cargo paths do not bypass it either. For a build that genuinely must run on this machine (for example the Postgres/MySQL integration suites against local Docker services, which remote workers cannot reach), use the shim's documented operator escape hatch for that one invocation: `RCH_SHIM_LOCAL_IDE=1 RCH_LOCAL_MAX_JOBS=3 cargo test ...` (run from a script so the PreToolUse hook does not reroute it), and say so in your report.
